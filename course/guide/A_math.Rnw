...
<<fun_xpowerk, fig.cap='Visualization of the power functions $y = x^k$ for $k = [1,2,3]$. In the case where $k = 1$, the growth of $y$ is linear because $y = x$. All three curves are examples of strict monotonicity, i.e. a systematic increase of $y$ when $x$ increases.'>>=
qplot(c(0, 5), 
      stat = "function", 
      fun = function(x) {
        x^1
...
<<fun_expgrowth, fig.cap='Exponentials.'>>=
qplot(c(-1, 3), stat="function", fun=function(x) {1^x}, geom="line", xlab="x", ylab="y") + 
...
<<fun_expdecay, fig.cap='Positive fractional exponentials: decay.'>>=
qplot(c(-1, 1), stat="function", fun=function(x) {1^x}, geom="line", xlab="x", ylab="y") + 
...
<<fun_powerexp, fig.cap='Power vs. exponential functions.'>>=
qplot(c(-5, 5), stat="function", fun=function(x) {2^x}, geom="line", xlab="x", ylab="y") + 
...
<<fun_explog, fig.cap='Logarithmic vs. exponential functions.'>>=
qplot(c(.1, 5), stat="function", fun=function(x) {x}, geom="line", xlab="x", ylab="y") + 
...
<<fun_expln, fig.cap='Natural logarithm vs. exponential functions.'>>=
qplot(c(-exp(1), exp(1)), stat="function", fun=function(x) {x}, geom="line", xlab="x", ylab="y") + 
