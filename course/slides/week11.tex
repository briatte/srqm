...
\begin{document}
	...
	\begin{frame}[t]{Final paper}
		...
		\begin{block}{3-step guide}
			...
			\begin{enumerate}
				\item \textbf{\red{Add regression models}} to your current draft
  ...
  %
	...
	\begin{frame}[t]{Technical requirements (1)}
        ...
        ... using e.g. the \code{stab} command
				\item A \textbf{regression results} table\\
        ... using e.g. the \code{eststo} and \code{esttab} commands
  ...
  %
	...
	\begin{frame}[t]{Technical requirements (2)}
		...
		\begin{block}{For \red{linear} regression models}

			\begin{enumerate}
				\item A linear model with \code{reg}
				\item Diagnostics of \textbf{residuals} (\code{predict}) and \textbf{variance} (\code{vif})%
				\item Interactions (\code{i.}, \code{\#}, \code{\#\#}, \code{c.})
			\end{enumerate}
			
		\end{block}

		\begin{block}{For \red{logistic} regression models}

			\begin{enumerate}
				\item A linear approximation with \code{reg}
				\item A \textbf{logistic model} with \code{logit [, or]}
				\item Interactions (\code{i.}, \code{\#}, \code{\#\#}, \code{c.})
			\end{enumerate}
  ...
  %
	...
	\begin{frame}[t]{Technical requirements (3)}
		...
		\begin{block}{\red{Marginal effects} (\code{margins}, \code{marginsplot})}
		...
		\begin{block}{More difficult \red{model specifications}}

			\begin{itemize}
        \item Weeks~10 and~11: clustered standard errors (\code{vce(cluster)})
        \item Week~11: bootstrapping (\code{vce(bootstrap)})
        \item Week~10: ordered logit (\code{ologit})
        \item Week~12: negative binomial regression (\code{nbreg})
			\end{itemize}
  ...
  %
	...
	\begin{frame}[t]{How to get there (1): \red{template do-files}}

		\begin{block}{Week~9}

			\begin{itemize}
				\item \textbf{Linear regression} with dummies and interactions
        \item \red{Diagnostics} using residuals and variance inflation
        \item \red{Exporting models} with \code{eststo} and \code{esttab}
			\end{itemize}
  ...
  %   \end{columns}
  % \end{frame}
	\begin{frame}[t]{The \red{truly hard part}}
		\begin{columns}[T]
			\column{.5\textwidth}
			...
			Then, \textbf{\red{interpret your relevant results}}, drawing on the weekly \textbf{readings} for help and on example \textbf{papers} for examples of how to report findings.\vspace{1em}%
