...
* Log.
cap log using "template2.log", name(draft2) replace
...
gr bar bmi, over(health) asyvars over(female) ///
	...
	scheme(burd5) name(age_health, replace)
...
spineplot bmi7 race, scale(.7) scheme(burd7) name(bmi7, replace)
...
* Export with stab. Note the new options to obtain correlation matrix of the
* continuous variables and t-tests for the categorical variables.
stab using template2 [aw=perweight], ///
	su(bmi age) corr ///
	fr(female edu3 health phy4 race hins) ttest ///
	replace
...
sc bmi age if age < 50 & hins, scheme(s2color) ///
...
* This is actually just like:
reg bmi i.age4 female hins i.race // multiple linear regression, our end goal!
