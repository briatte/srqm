...
All files are provided in `dta` Stata format on an "as-is" basis: please use them for teaching purposes only, and do not redistribute them for any reason. Modifications to the original files are listed in [`build.do`](https://github.com/briatte/srqm/blob/master/Datasets/build.do).
