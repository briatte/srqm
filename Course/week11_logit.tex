...
\usetheme[pageofpages=of,bullet=square,
	...
	titlepagelogo=images/table-extract.png]{Torino}
...
%			… and a lot more awesomeness later on with linear correlation and regression modelling.
		\column{.45\textwidth}
			\begin{center}
				\href{http://ideas.repec.org/c/boc/bocode/s447101.html}{\includegraphics[width=\textwidth]{images/table.png}}
			\end{center}		
		\end{columns}
		\vspace{2em}
	\end{frame}
	...
	\begin{itemize}
		\item Use \code{gr dot} to compare the mean, quantile or proportion of a \red{continuous variable} across groups.
		
		\begin{itemize}
			\item Some interval scales can be treated as pseudo-continuous.
			% e.g. 4-pt ``agree/disagree'' or 10-pt ``left/right'' scales.
			\item Use the \code{sort(1)des} option to order the groups by descending order.
			\item Use the \code{ylab()} option to label the horizontal axis correctly.
			\item Use the \code{exclude0} option if the null value is not meaningful.
		\end{itemize}
		
		\item Use \code{gr bar} or \code{gr hbar} with the \code{per stack} options to compare proportions of an \red{ordinal variable} as percentages across groups.
		
		\begin{itemize}
			\item Use \code{tab, gen()} to create dummies of each group category.
			\item Use the \code{legend(lab(\# "..."))} option to rewrite the legending.
			\item Use the \code{bar(\#,col(...))} option to apply a sensible color scheme.
			\item Use the \code{blab(bar, pos(center) format(\%9.2f))} to add labels.
		\end{itemize}
		
		\item Play around with \code{over()}, \code{by()} and other graph options, looking at course do-files for examples.
	\end{itemize}
	...
	\begin{frame}[t]{Operationalization: Pies}
		...
		Technically, you might want to use pie charts with the \code{by()} option to look at proportions over categories, as they  do at \textit{The Economist}:\\[1em]

		\href{http://andrewgelman.com/2011/04/attractive_but/}{\includegraphics[width=\textwidth]{images/uglyassgraph.jpg}}
	...
	\begin{frame}[t]{A Modest Proposal For Pie Charts (Hmm. Pie.)}
		...
		\begin{columns}[T]
			...
			\column{.6\textwidth}
			\includegraphics[width=\textwidth]{images/pie-problem.jpg}
		\end{columns}
	...
	\fullslide{images/crosstabs-s6.pdf}
...
Extreme-right 	& 6.4 	& 33.7 	& 29.5 	& 10.4\\
	...
	\includegraphics[width=\textwidth]{images/comp-prop3.pdf}\\[1em]
